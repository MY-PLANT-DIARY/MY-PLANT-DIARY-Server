...
public class PostController {
    ...
    public ResponseEntity posts(@PathVariable("userId")Long userId){
        ...
        List<PostResponseDto> collect = posts.stream()
                .map(post -> new PostResponseDto(post.getId(), post.getPlant().getPostCount(),post.getPostDate(), "http://localhost:8080/static/" + post.getImgUrl(), post.getText()))
    ...
    public ResponseEntity postReadOne(@PathVariable("postId")Long id) {
        ...
        PostResponseDto postResponseDto = new PostResponseDto(post.getId(), post.getPlant().getPostCount(), post.getPostDate(), "http://localhost:8080/static/" + post.getImgUrl(), post.getText());
